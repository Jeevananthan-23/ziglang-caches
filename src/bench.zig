...
pub fn main() !void {
    ...
    if (std.mem.startsWith(u8, "s3fifo", filter)) {
        try benchmark(
            @import("s3fifo/s3fifo.zig").BenchmarkS3fifo,
            max_time_per_bench,
            TimeUnits.milliseconds,
        );
    }

    if (std.mem.startsWith(u8, "lru", filter)) {
        try benchmark(
            @import("lru/lru.zig").BenchmarkLRUCache,
            max_time_per_bench,
            TimeUnits.milliseconds,
        );
    }

    if (std.mem.startsWith(u8, "sieve", filter)) {
        try benchmark(
            @import("sieve/sieve.zig").BenchmarkSieve,
            max_time_per_bench,
            TimeUnits.milliseconds,
        );
    }
...
pub fn benchmark(
    ...
    const min_width = blk: {
        ...
        res = try printBenchmark(
            ...
            formatter("Hit_Rate{s}", ""),
            formatter("Miss_Rate{s}", ""),
        );
        inline for (functions) |f| {
            ...
            while (i < args.len) : (i += 1) {
                ...
                res = if (i < arg_names.len) blk2: {
                    ...
                    break :blk2 try printBenchmark(writer, res, f.name, arg_name, max, max, max, max, max, max, max);
                } else blk2: {
                    break :blk2 try printBenchmark(writer, res, f.name, i, max, max, max, max, max, max, max);
                };
            }
        }
        break :blk res;
    ...
    inline for (functions, 0..) |def, fcni| {
        ...
        inline for (args, 0..) |arg, index| {
            ...
            const hits = @field(B, "hits");
            const misses = @field(B, "misses");

            if (index < arg_names.len) {
                const arg_name = formatter("{s}", arg_names[index]);
                _ = try printBenchmark(stderr, min_width, def.name, arg_name, i, min, max, variance, runtime_mean, hits, misses);
            } else {
                _ = try printBenchmark(stderr, min_width, def.name, formatter("{s}", ""), i, min, max, variance, runtime_mean, hits, misses);
            }
            try stderr.writeAll("\n");
            try stderr.context.flush();
        }
...
fn printBenchmark(
    ...
    try writer.writeAll(" ");
    const hit_rate = try alignedPrint(writer, .right, min_widths[5], "{}", .{hits});
    try writer.writeAll(" ");
    const miss_rate = try alignedPrint(writer, .right, min_widths[5], "{}", .{misses});

    return [_]u64{ name_len, it_len, min_runtime_len, max_runtime_len, variance_len, mean_runtime_len, hit_rate, miss_rate };
}
