...
pub const BenchmarkSieve = struct {
    ...
    pub fn benchmarkSieveGetandSet(num_iterations: usize) !void {
        const sieve = Sieve(.locking, u32, usize);
        var cache = try sieve.init(std.heap.page_allocator, 10);
        ...
        for (0..num_iterations) |_| {
            ...
            var node = sieve.Node{ .key = num, .value = num };
            const val = cache.get(node.key);
            if (val != null) {
                hits += 1;
            } else {
                misses += 1;
                _ = try cache.set(&node);
            }
        }
    }
};
